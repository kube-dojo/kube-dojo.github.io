...
**Pause and predict:** Before running this on a real node, what output do you expect from `file /etc/passwd /etc/ssh/sshd_config /etc/hosts`, and which result would make you slow down?

<details>
<summary>Check your prediction</summary>

If a supposedly normal configuration file appears as a symlink, device, or binary blob, that is not automatically wrong, but it changes your next move. You would inspect the target, package ownership, and service documentation before replacing it with a local edit.
</details>
...
The security angle is easy to underestimate. Logs often contain usernames, internal hostnames, request paths, failed authentication attempts, error payloads, and sometimes accidental secrets from application exceptions.

**Pause and predict:** What would happen if a debug log under `/var/log` captured an authorization header or a customer identifier — and what would a real fix require beyond deleting one file?

<details>
<summary>Check your prediction</summary>

The fix is not merely deleting one file; you would tighten application logging, rotate and expire existing files, restrict permissions, and send the corrected stream to a controlled logging backend.
</details>
...
The worked diagnostic pattern is to start with the least invasive read-only interface. If a network interface appears down, inspect `/sys/class/net`, read counters in `/proc/net/dev`, and then check service logs under `/var/log` before writing a sysctl or restarting services.

**Pause and predict:** Which approach would you choose first on a production node: changing a kernel setting under `/proc/sys/net`, or collecting `/proc`, `/sys`, and log evidence?

<details>
<summary>Check your prediction</summary>

The evidence-first approach protects you from turning an unclear incident into a self-inflicted one. Collect the live kernel and log evidence before writing a sysctl or restarting services.
</details>
...
Inode pressure produces a distinctive failure mode. A filesystem can show gigabytes free in `df -h` while `touch newfile` fails because there are no free inodes left to describe new files. That usually happens in directories full of tiny files: session stores, package cache fragments, mail spools, extracted dependency trees, or application-generated scratch files.

**Pause and predict:** If inode usage is exhausted but space remains, would deleting one huge file solve the problem?

<details>
<summary>Check your prediction</summary>

Usually not, because the problem is the count of file records, not the amount of stored data. Freeing inodes requires deleting or consolidating many small files (or expanding the filesystem with more inode capacity), not reclaiming a single large object's blocks.
</details>
...
Memory-backed filesystems illustrate why the path name alone is not enough. A `tmpfs` mounted at `/tmp/ramdisk` behaves like a directory, but its contents live in memory and disappear when unmounted or rebooted. That makes it useful for fast scratch data and tests, but dangerous for anything that must survive.

**Pause and predict:** What will `df -h /tmp/ramdisk` show after the mount, and what will `cat /tmp/ramdisk/test.txt` do after unmounting?

<details>
<summary>Check your prediction</summary>

After the mount, `df -h` should show a `tmpfs` filesystem with a capped size (for example 100M). After unmount, the path is an ordinary empty directory again (or gone if removed), so `cat /tmp/ramdisk/test.txt` fails — the file lived only in memory and did not persist to disk.
</details>
...
echo "scratch note for the lab" > /tmp/ramdisk/test.txt
...
### Part 5: Diagnostic Triage Challenge (Frozen Incident Cards)

Parts 1–4 stay as the evidence-collection recipe. These cards freeze the transcript so nothing needs to run. For each card, name the failure layer and one next action before opening the reveal.

**Card A: DiskPressure with free blocks.** `df -h /` shows 40% used, but `k describe node` reports DiskPressure and kubelet is evicting pods. `df -i /` shows inodes at 100%.

<details>
<summary>Failure layer and next action</summary>

Failure layer: inode exhaustion, not block capacity. Next action: identify directories with huge file counts (`find`/`du --inodes`), reclaim tiny-file caches/spools, then confirm `df -i` before assuming a larger disk will help.
</details>

**Card B: Deleted file still holding space.** `du -sh /var/log` looks small, but `df -h /var` still shows a full filesystem. `lsof +L1` lists a deleted `app.log` held open by PID 2210.

<details>
<summary>Failure layer and next action</summary>

Failure layer: open deleted file descriptor — space is released only when the last handle closes. Next action: rotate/reopen logs for that process (or restart the unit) so the deleted inode can free blocks; do not keep deleting filenames while the FD stays open.
</details>

**Card C: Database on tmpfs.** An engineer mounts `tmpfs` at `/var/lib/myapp` "for speed." After a reboot the database is empty and the service crashes on missing files.

<details>
<summary>Failure layer and next action</summary>

Failure layer: memory-backed mount used for durable state. Next action: remount a real persistent filesystem (or restore from backup) under `/var/lib/myapp`, and treat `tmpfs` as scratch-only in the platform baseline.
</details>

**Card D: Host path vs container mount.** On the host, `/var/log/app/error.log` has the smoking-gun lines, but `kubectl exec` into the pod shows an empty `/var/log/app`. The pod mounts a projected volume at that path.

<details>
<summary>Failure layer and next action</summary>

Failure layer: mount-namespace / volume wiring — the host path and the container path are different views. Next action: compare host `findmnt` with the pod's volumeMounts and inspect the projected volume contents instead of editing the host file the container never sees.
</details>
...
- [ ] Classified each frozen diagnostic card (failure layer + next action) before opening the reveal.
