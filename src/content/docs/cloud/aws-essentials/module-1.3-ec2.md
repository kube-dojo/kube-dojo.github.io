...
ec2_fleet_preflight() {
  ...
  if [[ -n "${VPC_ID:-}" ]]; then
    printf 'Using VPC_ID=%s from your environment (for example, Module 1.2 output).\n' "$VPC_ID"
    return 0
  fi
  VPC_ID=$(aws ec2 describe-vpcs --filters "Name=isDefault,Values=true" \
    --query 'Vpcs[0].VpcId' --output text) || return 1
  if [[ -z "$VPC_ID" || "$VPC_ID" == "None" ]]; then
    printf 'No default VPC in %s; export the VPC and subnet IDs from Module 1.2 before continuing.\n' "$AWS_REGION" >&2
    return 1
  fi
  ...
  if [[ "$DEFAULT_SUBNET_COUNT" -lt 2 ]]; then
    printf 'Default VPC %s has fewer than two default subnets; use the Module 1.2 subnets instead.\n' "$VPC_ID" >&2
    return 1
  fi
