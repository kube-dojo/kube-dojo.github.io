...
Identity incidents span two control planes, so playbooks that only mutate a single Azure identity object leave running replicas with leftover capability until operators also cycle the workload or change data-plane permissions on the target resource itself.
...
Shared service accounts are a convenience that becomes a blast-radius multiplier when every consumer in the namespace inherits the same Azure principal. Operators should treat identity enablement as a per-workload contract, not a namespace default that silently covers sidecars and batch jobs.
...
Credential rotation calendars fail when application code and the mount path disagree about who owns freshness. Teams that assume the orchestrator will bounce processes on every Key Vault version change discover stale connections only during the next incident.
