...
The difference matters most when the failure is logical. A ransomware process that encrypts files, a bad migration that corrupts records, and a privileged operator who deletes a resource each need a named recovery family before anyone picks a portal blade. A mature production design usually needs more than one family because logical loss and regional loss are different failure families.
...
| Bad release      | Named recovery point           | Named failover runbook       |
...
Worked example: a VM hosts a small order-processing worker. The VM has a daily backup at midnight with seven days of retention. At 15:00, an operator deletes a configuration directory and the service fails. The backup RPO is roughly the time between midnight and the delete, and the RTO includes restore, validation, and application repair. If the same VM is protected by Site Recovery with healthy replication, a regional outage might be handled by failover. The deleted-directory case is a different ticket: name the recovery point you would inspect before you name the region you would fail over to.
...
The **backup and point-in-time recovery-point family** is missing. Site Recovery continuously replicates disk writes and creates crash-consistent recovery points every five minutes, keeping them for the replication-policy retention window (default 24 hours; you can set a custom value between 0 and 72 hours). A failover to the latest point after an hour of corrupt writes follows that corrupt stream. Independent Azure Backup recovery points are the family that would have existed outside that short replication window.
</details>
...
SQL Server in Azure VM backup is not the same as backing up the VM disk. Azure Backup offers a stream-based workload-aware solution for SQL Server running in Azure VMs, with full, differential, and log backup support, database-level restore, and a much tighter RPO than a once-daily VM snapshot [SQL Server in Azure VM backup](https://learn.microsoft.com/en-us/azure/backup/backup-azure-sql-database). The operator decision is which restore granularity the ticket actually needs, and whether both families exist before the first corruption.
...
**Pause and predict:** A virtual machine is protected by a daily Azure VM backup schedule, while a critical SQL Server instance inside the guest is protected by a fifteen-minute log backup policy. When an operator accidentally corrupts a single database table during a midday schema migration, which recovery path should you execute, and what evidence would you collect before restoring?

<details>
<summary>Check your prediction</summary>

Use **SQL Server in Azure VM workload backup and database-level point-in-time restore (PITR)** from the transaction log chain instead of a full virtual machine restore. An in-place Azure VM restore replaces the entire guest operating system and reverts sibling databases and system files to last night's recovery point. Individual database restore applies the continuous transaction log chain to a point during the log-backup window before the migration, preserving the operating system and neighboring production databases.
</details>
...
Failure layer: Continuous replication engine versus point-in-time backup recovery-point boundaries. Next action: do not treat the latest Site Recovery replica as a clean standby; identify the exact pre-corruption timestamp, and restore from an Azure Backup recovery point in the Recovery Services vault taken prior to the faulty deployment.
</details>
...
Failure layer: Recovery Services vault control-plane and data-plane service isolation boundaries. Next action: configure dedicated Site Recovery private endpoints, target storage accounts, and private DNS zones (`privatelink.siterecovery.windowsazure.com`), recognizing that the Azure Backup private endpoint is dedicated exclusively to Backup management and data movement operations and does not route Site Recovery replication.
</details>
